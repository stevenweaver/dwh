 ...
 */
export default function computeDWH(
  network: any,
  binBy: any,
  value: any,
  randomize: boolean
  ...
  return (
    (WM + WC - 2 * WX) / (dIn / nodesIn / nodesIn + dOut / nodesOut / nodesOut)
  );
}
...
export function computeFractions(network: any, bin: any, randomize: boolean) {
  ...
  _.each(network["Edges"], (e) => {
    ...
    pairwiseConnections[sourceType][targetType] +=
      1 / targetDegree / sourceDegree;
    pairwiseConnections[targetType][sourceType] +=
      1 / sourceDegree / targetDegree;
  });
